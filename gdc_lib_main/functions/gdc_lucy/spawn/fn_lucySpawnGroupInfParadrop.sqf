/*
	...
	Returns:
	The group which will be paradropped
*/

params["_pos_spawn", "_pos_drop", "_pos_exit", "_side", "_plane_config", "_infantry_units"];
private["_cleaned_pos_spawn", "_next_waypoint_pos", "_plane_spawn", "_grp_inf", "_drop_statement", "_plane_altitude", "_plane_speed", "_plane_classname", "_plane_pilot", "_computed_angle"];
...
// Manage plane config
_plane_classname = _plane_config select 0;
_plane_pilot = _plane_config select 1;
_plane_altitude = _plane_config select 2;
...
// Add waypoint for all next positions
{
	...
	[_plane_spawn select 0, _next_waypoint_pos, 0, "MOVE", "LIMITED", "STEALTH", "BLUE", "RANDOM", 50.0] call GDC_fnc_lucyAddWaypoint;
} forEach _pos_spawn;
...
";
[_plane_spawn select 0, _jump_pos, 0, "MOVE", "LIMITED", "STEALTH", "BLUE", "RANDOM", 30.0, [0, 0, 0], ["true", _drop_statement]] call GDC_fnc_lucyAddWaypoint;
...
// Add a death waypoint at 10km, aligned with the two last waypoints
_computed_angle = [
	waypointPosition [_plane_spawn select 0, (count (waypoints (_plane_spawn select 0))) - 2],
	waypointPosition [_plane_spawn select 0, (count (waypoints (_plane_spawn select 0))) - 1]
] call BIS_fnc_dirTo;
_next_waypoint_pos = [
	((waypointPosition [_plane_spawn select 0, (count (waypoints (_plane_spawn select 0))) - 1]) select 0) + 1000 * sin(_computed_angle),
	((waypointPosition [_plane_spawn select 0, (count (waypoints (_plane_spawn select 0))) - 1]) select 1) + 1000 * cos(_computed_angle),
	_plane_altitude
];
[_plane_spawn select 0, _next_waypoint_pos, 0, "MOVE", "LIMITED", "STEALTH", "BLUE", "RANDOM", 50.0, [0, 0, 0], ["true", "{ deleteVehicle (vehicle _x); deleteVehicle _x; } forEach units group this;"]] call GDC_fnc_lucyAddWaypoint;
