	...
	if (
...

		{
			...
			if (
				...
				and ((_Target_Side getFriend _HQ_side)<0.6)
				and alive(leader(group _Target))
				and !captive(_target)
				//How sure are we? For now accuracy of 18 seems to work ok
				and _Target_PosAcc < 18
				and (_Target iskindof "AllVehicles")
				//If the lead has a clear LOS to a unit, that is valid enough (might expand later to check on all members)
				//and ([_unit,_Target,1500] call GDC_gaia_fnc_hasLineOfSight)
			) then {
...
for [{_i=0},{_i < count _ToBeCleared},{_i=_i+1}] do {
	...
	if ([(_OldTargetInfo select 1),_HQ_side ] call GDC_gaia_fnc_isCleared) then {
