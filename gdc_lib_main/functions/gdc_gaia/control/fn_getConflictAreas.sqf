...
switch (_HQ_side) do {
	case west: {
		_Targets = (MCC_GAIA_TARGETS_WEST select 0)+(MCC_GAIA_TARGETS_WEST select 1)+(MCC_GAIA_TARGETS_WEST select 2);
		_Spots = (MCC_GAIA_TARGETS_WEST select 3);
		_CA = MCC_GAIA_CA_WEST;
	};
	case east: {
		_Targets = (MCC_GAIA_TARGETS_EAST select 0)+(MCC_GAIA_TARGETS_EAST select 1)+(MCC_GAIA_TARGETS_EAST select 2);
		_Spots = (MCC_GAIA_TARGETS_EAST select 3);
		_CA = MCC_GAIA_CA_EAST;
	};
	case independent: {
		_Targets = (MCC_GAIA_TARGETS_INDEP select 0)+(MCC_GAIA_TARGETS_INDEP select 1)+(MCC_GAIA_TARGETS_INDEP select 2);
		_Spots = (MCC_GAIA_TARGETS_INDEP select 3);
		_CA = MCC_GAIA_CA_INDEP;
	};
	case civilian : {
		_Targets = (MCC_GAIA_TARGETS_CIV select 0)+(MCC_GAIA_TARGETS_CIV select 1)+(MCC_GAIA_TARGETS_CIV select 2);
...
//_ca=[_ca,[],{[_HQ_side,_x] CALL GDC_gaia_fnc_getDistanceToClosestZone;}, "ASCEND"] call BIS_fnc_sortBy;
if (count(_ca)>0) then {
	_CA=[_CA,[],{[_HQ_side,_x] CALL GDC_gaia_fnc_getDistanceToClosestZone;}] call BIS_fnc_sortBy;
};
