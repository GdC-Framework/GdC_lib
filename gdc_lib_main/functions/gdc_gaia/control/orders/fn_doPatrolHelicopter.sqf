...
if !(isnil("_Zone")) then {
    ...
    if ( ((_group) getVariable ["GAIA_LandingSpot",[0,0,0]] distance [0,0,0]==0) ) then {
        ...
        if (isTouchingGround (assignedvehicle leader _group)) then {
            ...
            _landpos = [([_zone, "ARM_HILLS_FLAT",(side _group)] call GDC_gaia_fnc_calculateOptimalPosition), 0,100, 12, 0, 60 * (pi / 180), 0] call BIS_fnc_findSafePos;
    ...
    if (
        ...
        and !_initial
        and (count(_group getVariable ["GAIA_zone_intend",[]])>1)
        and ((_group) getVariable ["GAIA_LandingSpot",[0,0,0]] distance [0,0,0]!=0)
    ) then {
        ...
        for "_x" from 1 to _rounds do {
            ...
            if (((_group getVariable ["GAIA_zone_intend",[]]) select 1)=="NOFOLLOW") then {
                _pos = ([_zone , "AIR",(side _group)] call GDC_gaia_fnc_calculateOptimalPosition);
            } else {
                _pos = ([(_zns call BIS_fnc_selectRandom ) , "AIR",(side _group)] call GDC_gaia_fnc_calculateOptimalPosition);
            };
