...
if !(surfaceiswater _TargetPos ) then {
	...
	while {((_Pos distance _TargetPos)>200) and _count < 6} do {
		...
		if !(surfaceiswater _pos ) then {
			...
			_dummy = [_group,_pos, "MOVE"] call GDC_gaia_fnc_addAttackWaypoint;
			_count = _count + 1;
		};

	};

	_dummy = [_group,_TargetPos, "SAD"] call GDC_gaia_fnc_addAttackWaypoint;
	//_NrOfBuildingWp = [_group]call GDC_gaia_fnc_generateBuildingPatrolWaypoints;
};
