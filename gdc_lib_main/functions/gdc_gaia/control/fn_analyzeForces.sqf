...
//Get all zones that still hold forces.
{
	if (
		...
		if (count([_x] call GDC_gaia_fnc_getZoneIntendOfGroup)>0) then {_TempGroups = _TempGroups + [_x];};
...
{
	_zone = _x;
	_ZoneBehave	=([_Zone,_side] call GDC_gaia_fnc_getZoneStatusBehavior);
	_ParentStatus = (_ZoneBehave select 0);
	{

		_ZoneBehave	=([_x,_side] call GDC_gaia_fnc_getZoneStatusBehavior);
...
//Also check the progress of a unit. Is he a candidate stuck unit?
{
	if (
		...
		// and (behaviour _x !="COMBAT")
   		...
   		_ZoneIntend = [(_x)] call GDC_gaia_fnc_getZoneIntendOfGroup;
   		if (count(_zoneintend)>0) then {
   		//Zone info on group found.
			_ZoneBehave	=([(_zoneintend select 0),_side] call GDC_gaia_fnc_getZoneStatusBehavior);

			//We even found the expected bheavior.
			if (count(_zonebehave)>0
				and (behaviour leader _x	!="COMBAT")
				and (behaviour leader _x	!="STEALTH")
