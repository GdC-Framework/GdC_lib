            ...
            if (
                         ...
                         and
                          ...
                          and
                           ...
                           and
                           ((_x getVariable  ["GAIA_Order",""]) != "DoWait")
                          and
                         (behaviour(leader _x)=="COMBAT")
                         and
                         !((([_HQ_side,([_CA,leader _x] call BIS_fnc_nearestPosition)] call GDC_gaia_fnc_getConflictAreaCost) select 0 )in ["Helicopter","Autonomous"])
                    )
...

{
    ...
    //Now for each group make sure we check what he needs to do.
    {
         ...
         then
         {
                ...
                _GroupBusy = (((_x getVariable  ["GAIA_OrderPosition",[0,0,0]]) distance _SelectCA)==0)
                             or
                              ((_x getVariable  ["GAIA_Order",""]) in ["DoTransport"]) ;
                ;
             ...
             then
             {
                    ...
                    _MortarFired = false;
                    {
                        ...
                        if  (
                            ...
                            !((([_HQ_side,_SelectCA] call GDC_gaia_fnc_getConflictAreaCost)select 0) in ["Car","Tank","Helicopter","Ship","Support","Autonomous","Submarine"])
                            and
...
// = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
{
    if (count(_x getVariable  ["GAIA_zone_intend",[]])>1)       then
    {
        if (
            ...
            ((side _x) == _HQ_side)
            and //There must be units in the group of the requester
            (count(units _x)>0)
            and //The group cannot be in combat
            ((behaviour leader _x)!="COMBAT")
            and //The leader must be alive (null group check)
            (alive (leader _x))
            and //There must be something to do in your portfolio
            (count(_x getVariable  ["GAIA_Portfolio",[]])>0)
            and (
                (
        ...
        then  { _dummy= [_x] call GDC_gaia_fnc_doTransport};
    };
