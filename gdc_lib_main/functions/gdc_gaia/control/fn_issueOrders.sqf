...

{
    if (
        ...
        if (
            ...
            and ((_x getVariable ["GAIA_Order", ""]) != "DoMorter") // TODO: Check if it's not DoMortar
            //He has actualy opened up his mortar rounds by atleast first cycle
            and (_x getVariable ["GAIA_MortarRound",0]>0)
        ) then {
            player globalchat "Mortar cancel 1";
            [_x] call GDC_gaia_fnc_removeWaypoints;
        ...
        if (
            ((_x getVariable ["GAIA_class", "None"])!=(_x getVariable ["GAIA_PreviousClass", "None"]))
            //and
            //!((_x getVariable ["GAIA_Order", ""]) in ["DoTransport"])
        ) then {[_x] call GDC_gaia_fnc_removeWaypoints;player globalchat "Class change 2";};
        ...
        if (
            ...
            and ((_x getVariable ["GAIA_Order", ""]) != "DoAttack")
            and ((_x getVariable ["GAIA_Order", ""]) != "DoFortify")
        ...
        if (
            ...
            and (_x getVariable ["GAIA_Order", ""])!=((_x getVariable ["GAIA_Combinedorder",grpNull]) getVariable ["GAIA_Order", ""])
        ) then {
            [_x] call GDC_gaia_fnc_removeWaypoints;
            player globalchat "On combined order, the group that is combined must be on same order";
        };
        ...
        if (
            ...
            if ((((_x getVariable ["GAIA_CombinedOrder",grpNull]))getVariable ["GAIA_CombinedOrder",grpNull])!=_x) then {
                [_x] call GDC_gaia_fnc_removeWaypoints;
                player globalchat "The group where i combine with must be combined to me";
            };
        };
        ...
        if (
            ...
            and ((_x getVariable ["GAIA_Order", ""]) != "DoWait")
            and (behaviour(leader _x)=="COMBAT")
            and !((([_HQ_side,([_CA,leader _x] call BIS_fnc_nearestPosition)] call GDC_gaia_fnc_getConflictAreaCost) select 0 )in ["Helicopter", "Autonomous"])
        ) then {[_x] call GDC_gaia_fnc_removeWaypoints;player globalchat "5"+format["%1", _x]+(_x getVariable ["GAIA_Order", ""]);};
        ...
        if (
            ...
            and ((_x getVariable ["GAIA_Order", ""]) != "DoFortify")
        ) then {[_x] call GDC_gaia_fnc_removeWaypoints;player globalchat "Timeout6";};

        //Candel DoAttack order that attacks a no longer existing CA
        if (
            !((_x getVariable ["GAIA_OrderPosition",[0,0,0]]) in _ca)
            and ((_x getVariable ["GAIA_Order", ""]) == "DoAttack")
        ) then {[_x] call GDC_gaia_fnc_removeWaypoints;player globalchat "7";};
    };
}forEach AllGroups;
...
// = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
{
    if (
        ...
        if (((_x getVariable ["GAIA_zone_intend",[]]) select 1)=="FORTIFY") then {
            _dummy=[_x,(position leader _x)] call GDC_gaia_fnc_doFortify;
        };
    };
...
// = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
{
    ...
    _CaPoints = ([_side,_SelectCA] call GDC_gaia_fnc_getConflictAreaCost) select 2;
     // Send a minimum of groups, even if points are met
    _NrGrpsSend = 0;


    //Lets figure out what is already send to this location
    _PointsSpend = 0;
    {
        if (
            //Seems like allgroups opens up with all sorts of empty groups, better check it
            ...
            if (((_x getVariable ["GAIA_OrderPosition",[0,0,0]]) distance _SelectCA)==0) then {
                _PointsSpend = _PointsSpend + (_x getVariable ["GAIA_points",0]);
    ...
    //Now for each group make sure we check what he needs to do.
    {
        if (
            ...
            if (count(_x getVariable ["GAIA_zone_intend",[]])>1) then {
                _CanDoAttack = (
                    ...
                    or (
                        ...
                        and (_SelectCA inArea ((_x getVariable ["GAIA_zone_intend",[]]) select 0))
                    )
                );
            } else {
                _CanDoAttack = false;
                ...
                {
                    if ([(position leader _grp ),_x] call GDC_gaia_fnc_isPositionInMarker) exitwith {_CanDoAttack=true};
                }foreach _zns;
                */
            };
            ...
            if ((_CanDoAttack) and (count(_x getVariable ["GAIA_zone_intend",[]])>1)) then {
                _CanDoAttack = (
                    (
                        ((_x getVariable ["GAIA_speed", ""])=="SLOW" and ((leader _x) distance _SelectCA)<(MCC_GAIA_MAX_SLOW_SPEED_RANGE*2))
                        or (_SelectCA inArea ((_x getVariable ["GAIA_zone_intend",[]]) select 0))
                    )
                    or (
            ...
            if (_CanDoAttack) then {
                ...
                _CanDoAttack = (
                    ...
                    and (
                        ...
                        or !(((_x getVariable ["GAIA_class", ""]) in ["Ship", "Submarine"]) and !(surfaceIsWater(_SelectCA)))
                    )
                );
            };
            ...
            _CaHoldsValidTargets = !((([_HQ_side,_SelectCA] call GDC_gaia_fnc_getConflictAreaCost)select 0) in ["Helicopter", "Autonomous"]);
            ...
            switch (true) do {
                ...
                case (
                    ...
                    _dummy=[_x] call GDC_gaia_fnc_removeWaypoints;
                    player globalchat "Group is fleeing";
                };
                ...
                case (
                    ...
                    _GrpIsInCombat
                    and _CanDoAttack
                    and _IsGaiaControlled
                    and _SelectCAisClosesttoGroup
                    and !_GroupHasOrdersThisRound
                    and !_GroupIsFleeing
                    and _CaHoldsValidTargets
                    and !_GroupBusy
                ) : {
                    _dummy=[_x,_SelectCA] call GDC_gaia_fnc_doAttack;
                    _PointsSpend = _PointsSpend + _Grpcost;
                    _NrGrpsSend = _NrGrpsSend + 1;
                    player globalchat "Group is attacking";
                };
                ...
                case (
                    ...
                    _PointsSpend = _PointsSpend + _Grpcost;
                    _NrGrpsSend = _NrGrpsSend + 1;
                    player globalchat "Group is attacking 2";
                };
            };
        };
    ...
    if (count(_SpotInfo)>0) then {
...

        {
            if (
                ...
                _SpotIsBeingCleared = [_SpotPos,_side] call GDC_gaia_fnc_isBlacklisted;
                ...
                _SpotIsWithinRange = (
                    ...
                    or (_SpotPos inArea ((_x getVariable ["GAIA_zone_intend",[]]) select 0))
                );
                ...
                switch (true) do {
                    ...
                    case (
                        ...
                        _dummy=[_x,_SpotPos] call GDC_gaia_fnc_doClear;
                    };
                };
            };
...
// = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
{
    ...
    if (
        ...
        and (
            ...
            or ("DoArtillery" in (_x getVariable ["GAIA_Portfolio",[]]))
        )
        //if we are already busy mortaring, forget the time out, ofcourse
        and (
            ((_x getVariable ["GAIA_MortarRound",0])>0)
            or ((time - (_x getVariable ["GAIA_OrderTime",-MCC_GAIA_MORTAR_TIMEOUT])) > MCC_GAIA_MORTAR_TIMEOUT)
        )
    ) then {
        _MortarFired = false;
        ...
        {
            _SelectCA = _x;
            ([_side,_SelectCA] call GDC_gaia_fnc_getConflictAreaCost) select 2;
            ...
            if (
                ...
                and !((([_HQ_side,_SelectCA] call GDC_gaia_fnc_getConflictAreaCost)select 0) in ["Car", "Tank", "Helicopter", "Ship", "Support", "Autonomous", "Submarine"])
                ...
                _MortarFired = [_grp,_SelectCA] call GDC_gaia_fnc_doMortar;
            } else {
        ...
        if (count (waypoints _x ) == (currentWaypoint _x)) then {
            ...
            switch (true) do {
                ...
                case ("DoWait" in (_x getVariable ["GAIA_Portfolio",[]])): {
                    //Clear the dude till he clears his mind.
                    //We do not count him into the attack
                    _dummy=[_x] call GDC_gaia_fnc_doWait;
                };
                ...
                case ("DoPatrol" in (_x getVariable ["GAIA_Portfolio",[]])): {
                    //Clear the dude till he clears his mind.
                    //We do not count him into the attack
                    _dummy=[_x] call GDC_gaia_fnc_doPatrol;
                };
                ...
                case ("DoSupport" in (_x getVariable ["GAIA_Portfolio",[]])): {
                    //Clear the dude till he clears his mind.
                    //We do not count him into the attack
                    _dummy=[_x] call GDC_gaia_fnc_doSupport;
                }; \
                ...
                case ("DoHide" in (_x getVariable ["GAIA_Portfolio",[]])): {
                    //Clear the dude till he clears his mind.
                    //We do not count him into the attack
                    _dummy=[_x] call GDC_gaia_fnc_doHide;
                };
                ...
                case ("DoPark" in (_x getVariable ["GAIA_Portfolio",[]])): {
                    //Clear the dude till he clears his mind.
                    //We do not count him into the attack
                    _dummy=[_x] call GDC_gaia_fnc_doPark;
                };
            };
        };
    };
    ...
    if (count(_x getVariable ["GAIA_zone_intend",[]])>1) then {
        if (
            ...
            and (
                ...
                or ((_x getVariable ["GAIA_Order", ""]) == "DoAttack")
            )
            and //You belong to a footmobile class or you wont be able to getin
            ...
            _dummy= [_x] call GDC_gaia_fnc_doTransport
        };
    };
} forEach allgroups;
        ...
        if (
            ...
            if ((_x getVariable ["GAIA_class", ""]) in ["Infantry", "ReconInfantry"]) then {
                ...
                if (_IsNight and count(_ca)>0 and ((_ClosestCA distance (leader _x))<400) and _LetsDoSome and (((leader _x) findNearestEnemy _PosLead) distance _PosLead >100) ) then {
                    [[(_ClosestCA select 0),(_ClosestCA select 1),100],[(_PosLead select 0),(_PosLead select 1),40],50,30, "F_40mm_Green"] call GDC_gaia_fnc_fireFlare;
                };
