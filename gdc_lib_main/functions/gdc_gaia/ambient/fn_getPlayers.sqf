...
while {(count _players) == 0} do {
  ...
  //Loop them
  {
		if ((_time +30) > time ) then {
			_dummy = [(position (_all select 0))] call GDC_gaia_fnc_ShowLocationOwner;
			_time = time;
		};
    ...
    if(isPlayer _x) then {
      if((alive _x) && (!captive _x))then {
        ...
        {
					if([position _player,str _x] call GDC_gaia_fnc_IsInMarker) exitWith { _spotValid = false; };
	...
	//Delete old shit
	{
		...
		if (_grp getVariable ["GAIA_AMBIENT",false]) then {
			{
				if ([position _x,_dissapearDistance] call GDC_gaia_fnc_isNearPlayer) exitWith {_delete = false;};
			} forEach (units _grp);
