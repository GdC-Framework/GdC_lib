...
//Build city locations
{
	...
	if (_oc_size>10) then {
		...
		_sideRatios = [(locationPosition _x)] call GDC_gaia_fnc_getsideratio;
		...
		if !(_set) then {
			_MarkerType ="mil_warning";_MarkerColor = "ColorWhite";_set=true;
		};

		if ((getMarkerType _mrk) != _MarkerType) then {
			_mrk setMarkerType _MarkerType;
		};
		if ((getMarkerColor _mrk) != _MarkerColor) then {
