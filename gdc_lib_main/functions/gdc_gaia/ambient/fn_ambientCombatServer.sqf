...
while {true} do
{
	...
	sleep _timeDelay;
	_syncedUnit = call GDC_gaia_fnc_GetPlayers;
	...
	if (
		...
		while {!_spotValid} do {
			...
			if (
				...
				and !([_spawnPos,_minRange] call GDC_gaia_fnc_isNearPlayer)
			) then {
				_spotValid = true;
			};
			...
			{
				if([_spawnPos,str _x] call GDC_gaia_fnc_IsInMarker) exitWith { _spotValid = false; };
		...
		_DoStuff = true;
		_sideRatios = [(position _player)] call GDC_gaia_fnc_getsideratio;
		...
		if (_DoStuff) then {
			_grp =[_sideRatios,[ _FactionsWest,_FactionsEast, _FactionsIndep],_spawnpos] call GDC_gaia_fnc_SpawnGroup;
		};
	};
};